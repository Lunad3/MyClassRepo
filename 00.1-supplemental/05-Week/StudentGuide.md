## Introduction to Node and NPM

### Overview

In this week, we will be introducing you to application building with Node and the use of external NPM packages.

### Homework Activities

* [Liri Bot](../../10-nodejs/02-Homework/Instructions/)

### Helpful Links

* [Basic Intro to Node](http://blog.modulus.io/absolute-beginners-guide-to-nodejs)
* [fs Documentation](https://nodejs.org/api/fs.html)
* [InquirerJS](https://www.npmjs.com/package/inquirer)

### Additional Course Resources

* [Curriculum Resources](https://github.com/coding-boot-camp/curriculum-resources)
